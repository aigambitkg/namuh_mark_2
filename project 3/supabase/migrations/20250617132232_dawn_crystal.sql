...
BEGIN
    ...
    INSERT INTO public.user_subscriptions (
        ...
        CASE WHEN v_user_role = 'applicant' THEN 2 ELSE 0 END, -- Only applicants get quiz uses
        CASE WHEN v_user_role = 'recruiter' THEN 2 ELSE 0 END, -- Only recruiters get job posting limits
        CASE WHEN v_user_role = 'recruiter' THEN 0 ELSE 0 END, -- Only paid recruiters get profile matcher searches
        CURRENT_TIMESTAMP
...
DO $$
BEGIN
    -- Check if policy exists for applicant profiles
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'applicant_profiles' 
        AND policyname = 'Users can view own applicant profile'
    ) THEN
    ...
    -- Check if update policy exists for applicant profiles
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'applicant_profiles' 
        AND policyname = 'Users can update own applicant profile'
    ) THEN
    ...
    -- Check if policy exists for recruiter profiles
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'recruiter_profiles' 
        AND policyname = 'Users can view own recruiter profile'
    ) THEN
    ...
    -- Check if update policy exists for recruiter profiles
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'recruiter_profiles' 
