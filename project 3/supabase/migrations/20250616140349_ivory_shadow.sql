/*
  # User creation and deletion handlers

  1. New Functions
    - `handle_new_user()` - Creates profiles and subscriptions for new users
    - `handle_user_deletion()` - Cleans up user data on deletion
    - `handle_auth_user_creation()` - Bridge function for auth.users trigger
    - `handle_auth_user_deletion()` - Bridge function for auth.users trigger
  
  2. Triggers
    - `on_auth_user_created` - Trigger on auth.users for new users
    - `on_auth_user_deleted` - Trigger on auth.users for deleted users
*/
